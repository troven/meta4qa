...
module.exports = function(config) {
    ...
    var self = this;
    self.scope = _.extend(config, new Events.EventEmitter());
    ...
    debug("Reporter: %s", config.reporter);
    ...
    this.onFeature = function(dialect, feature) {
        ...
        return new Feature(dialect, feature, self.scope).feature(self.suite);
    }
    ...
    this.run = function(done) {
        ...
        self.runner.run(function(failures) {
            ...
            if (failures>0) {
                ...
                debug("Run Failed: %s failures", failures);
