...
//    scope = _.extend(scope, config.vars);
    ...
    this.dialect = function(stepsDir) {
        ...
        debug("dialect %s ->: %j", stepsDir, _.keys(stepsFiles) );
...
//        debug("annotations: %s", _.keys(self.scope.phrases));
        return self;
    }
...
//        debug("feature-scoped: %s", _.keys(self.scope.phrases));
    ...
    this.execute = function(phrase, scope, done) {
        ...
        return self.yadda.run(phrase, scope, function() {
            self.scope.emit("phrase", phrase, scope);
            done && done();
        });
    }
