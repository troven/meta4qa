...
module.exports = function(config) {
    ...
    self.execute = function(config, onComplete) {
        if (_.isString(config)) {
            config = { features: self.config.paths.features +"/"+ config+".feature" };
        }
        ...
        featuresPath = config.features || config.paths.features || featuresPath;
    ...
    self.examples = function(from) {
        mkdirp(featuresPath);
        var configFile = __dirname+"/../examples/example_config.json";
        var config = helpers.files.config(configFile , self.safeConfig());
console.log("example config: %j", config);
