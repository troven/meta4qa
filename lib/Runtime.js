...
module.exports = function(config) {
    ...
    var self = this;
    self.config = config;
    var featuresPath = config.featuresPath || config.paths.features;
    ...
    self.execute = function(config, onComplete) {
        ...
        config = _.extend({ paths: {features: featuresPath } },self.config, config);

        onComplete = onComplete || self.onComplete;

        assert(onComplete, "Missing onComplete");

debug("executing: %j -> %s", config, featuresPath)
        var featuresPath = featuresPath || config.featuresPath || config.paths.features ;

        assert(featuresPath, "Missing Feature Path");
        assert(self.config.repeat, "Missing Repeat Count");

        debug("features: %s -> %j", featuresPath, self.config.paths);
        ...
        async.times(self.config.repeat, function (n, next) {
            ...
            // execute ...
            debug("Running #%s/%s  @ %s", (n+1), config.repeat, featuresPath);
