...
var self = module.exports = {
    _: _,
    debug: debugs("meta4qa:blueprint"),
    files: files,
    vars: vars,
    hbs: hbs,
    ...
    assets: function(dir, to, ctx, done) {
        ...
        debug("required: %s -> %s", pkg_dir, pkg_json);
        from = false;
        ...
        if (files.exists(pkg_json)) {
            ...
            ctx = _.extend({ pkg: { directories: {} } }, ctx, pkg);
            ...
            from = self.path(dir, ctx.pkg?ctx.pkg.directories.assets:"" );
        }
    ...
    copyFolder: function(from, to, ctx) {
        ...
        if (every && from.indexOf("{{")>=0) {
            ...
            debug("folders: %s -> %s @ %s -> %j", from, to, every, _.keys(loop) );

            _.each(loop, function(_ctx, key) {
                _ctx.key = key;
                var path = self.getFilename(from, to, key, _ctx);
                mkdirp.sync(path);
                debug("cp %s folder: %s -> %s => %j", key, from, path, _ctx);
    ...
    _copyTemplate: function(from, to, ctx) {
        ...
        mkdirp.sync(to_dir);
//console.log("TEMPLATE CTX: %j", _.keys(ctx));
        // magic {{..}} paths
