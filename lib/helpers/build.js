...
var self = module.exports = {
    ...
    assets: function(dir, to, ctx, done) {
        ...
        DEBUG && debug("PKG? %s @ %s",  packaged, pkg_json);
        if (packaged) {
            ...
            if (_.isFunction(pkg)) {
                ...
                DEBUG && debug("PKG(): %s (%s)",  pkg.pkg.name, pkg.pkg.author);
            } else {
            ...
            DEBUG && debug("partials: %j ", _.keys(folders));
        ...
        DEBUG && debug("build assets: %s -> %s", from, to);
    ...
    copy: function(from, to, ctx) {
        ...
        files.mkdirp(to);
        DEBUG && debug("cp folder: %j-> %s", froms, to);
        _.each(froms, function(file) {
            ...
            if (self.builder(from_file, to_file, ctx)) {
                DEBUG && debug("custom: %s", to_file);
                // done
            } else {
                DEBUG && debug("_cp: %s -> %s", from_file, to_file);
    ...
    copyFolder: function(from, to, ctx) {
        ...
        var everyFolder = function(_ctx, key) {
            ...
            mkdirp.sync(path);
            DEBUG && debug("cp %s folder: %s -> %s", key, from, path);
        ...
        if (every && from.indexOf("{{")>=0) {
            ...
            if (isSingular) {
                DEBUG && debug("folders {%s}: %s -> %s -> %j", every, from, to, loop );
                everyFolder(ctx, loop);

            } else {
                DEBUG && debug("folders {%s %s}: %s -> %s -> %j", every, qs ,from, to, _.keys(loop) );
                _.each(loop, everyFolder );
            }

        } else {
            files.mkdirp(to);
            DEBUG && debug("cp folder: %s -> %j", to, path);
            self.copy(from,to,ctx);
        }
    },
...
//        DEBUG && debug("cp templates: %s -> %s (%s)\n\t%j", from, to, loop, ctx);
        ...
        DEBUG && debug("_every: %s -> %j", every, loop);
        ...
        _.each(loop, function(_ctx, key) {
            ...
            files.mkdirp(dir);
            // DEBUG && debug("_mk{%s}: %s -> %s\n%j", dir, from, path, _ctx);

            var meta = _.extend({}, _ctx, { "key": key, self: ctx});
            var new_to = self.render(to, ctx);
            // DEBUG && debug("rendered: %s", new_to);
            var rendered = self.render(src, meta);
            if (rendered.trim()) {
                DEBUG && debug("_copyTemplates: %s", path); //meta
                self.save(new_to, rendered);
            } else {
                DEBUG && debug("_emptyTemplates: %s", path); //meta
            }
        });
