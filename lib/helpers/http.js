...
http.proxyURL = function (cmd, options) {
    ...
    var proxyCredentials = proxyUser + (proxyPass?":"+proxyPass:"");

    var proxyUrl = options.protocol + proxyCredentials + options.hostname+(port?":"+port:"");
    cmd.proxy = proxyUrl;
    debug("via %s PROXY: %s", options.protocol, proxyUrl);
}
...
http.command = function (method, resource, options, target) {
    ...
    debug("PROXY: %s -> %s", cmd,url, cmd.proxy)
