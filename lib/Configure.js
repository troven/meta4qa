...
//    config.paths.features = config.paths.features;
    ...
    var CONFIG_FILE = config.configFile = configFile || (cli.configFile || cli.config);
    ...
    debug("Features folder: %s", cli.features);
    ...
    if ( !foundFeatures && !pleaseSetup ) {
        console.log("Missing features path: %s - use %s--initialize to create.", config.paths.features, cli.name);
        process.exit(1);
        return;
    }
